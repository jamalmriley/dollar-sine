...
export default function Prefix() {
  ...
  const [displayNameType] = useQueryState("displayNameType", {
    defaultValue: "",
  });
  ...
  function getDisplayNameValue(displayNameType: string): string {
    const displayNameTypes = [
    ...
    for (const obj of displayNameTypes) {
      if (obj.type === displayNameType) return obj.value as string;
    }
    return "";
  }
          ...
          <div key={prfx} className="inline-block p-2">
            <button
              ...
              onClick={() => {
                ...
                setPrefix(prfx);
                if (displayNameType !== "") setDisplayNameValue(newDisplayName);
              }}
            >
              {prfx}
            </button>
